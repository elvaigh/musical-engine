...
model_filename = "bestmodel.hdf5"#model_basename + str(cur_iter)
...
#if os.path.isfile(model_filename):model.load_weights(model_filename)

num_iters = 5			# Number of iterations for training
epochs_per_iter = 1		# Number of iterations before we save our model
batch_size = 8			# Number of training examples pushed to the GPU per batch.
...
#if cur_iter%100==0:
#print("savizng model at iter:{}".format(cur_iter))
history = model.fit(X_train, y_train, batch_size=8, nb_epoch=0, verbose=1,validation_split=0.0)
if os.path.isfile(model_filename):model.load_weights(model_filename)

checkpoint = ModelCheckpoint(model_filename, save_best_only=True, monitor='loss', mode='min')
history = model.fit(X_train, y_train, batch_size=8, nb_epoch=50000,
                    verbose=1, validation_split=0.0,callbacks=[checkpoint])
